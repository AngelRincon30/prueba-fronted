...
export const useHandlersList = () => {
  ...
  const { data } = useQuery({
    queryKey: ["repoData"],
    queryFn: () =>
      fetch("https://jelou-prueba-tecnica1-frontend.rsbmk.workers.dev").then(
        (res) => res.json(),
      ),
  });
  ...
  useEffect(() => {
    if (!data) return;
    if (valueGender) {
      const result = data?.default?.library.filter(
        (x: any) => x?.book?.genre === valueGender,
      );
      setListBook(result);
    } else {
  ...
  const handleDelete = useCallback(
    (ID: string) => {
      ...
      myPersonLibraryBook?.length === 1 &&
        localStorage.removeItem("booksMyLibrary");
    },
    [myPersonLibraryBook],
  );
